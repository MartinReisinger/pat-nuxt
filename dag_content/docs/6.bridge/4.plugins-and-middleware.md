...
Plugins now take only one argument (`nuxtApp`). You can find out more in [the docs](/dag_content/docs/4.x/directory-structure/app/plugins).
...
::note
If you want to use the new Nuxt composables (such as [`useNuxtApp`](/dag_content/docs/4.x/api/composables/use-nuxt-app) or `useRuntimeConfig`) within your plugins, you will need to use the `defineNuxtPlugin` helper for those plugins.
::
...
Middleware now take only two argument (`to`, `from`). You can find out more in [the docs](/dag_content/docs/4.x/directory-structure/app/middleware).
...
You can also use [`definePageMeta`](/dag_content/docs/4.x/api/utils/define-page-meta) in Nuxt Bridge.
