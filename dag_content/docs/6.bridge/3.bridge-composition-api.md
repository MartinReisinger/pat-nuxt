...
- `withContext` has been removed. See [below](/dag_content/docs/4.x/bridge/nuxt3-compatible-api#usecontext-and-withcontext).
...
- `reqRef` and `reqSsrRef`, which were deprecated, have now been removed entirely. Follow the instructions below regarding [ssrRef](/dag_content/docs/4.x/bridge/nuxt3-compatible-api#ssrref-and-shallowssrref) to replace this.
...
You may also keep using Nuxt 2-style plugins, by removing the function (as with [defineNuxtMiddleware](/dag_content/docs/4.x/bridge/bridge-composition-api#definenuxtmiddleware)).
...
Nuxt Bridge provides direct replacements for these composables via [`useRouter`](/dag_content/docs/4.x/api/composables/use-router)  and `useRoute`.

The only key difference is that [`useRoute`](/dag_content/docs/4.x/api/composables/use-route) no longer returns a computed property.
