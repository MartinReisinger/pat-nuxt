...
If you need to access the component state with `head`, you should migrate to using [`useHead`](/dag_content/docs/4.x/api/composables/use-head) .
...
Nuxt Bridge provides a new Nuxt 3 meta API that can be accessed with a new [`useHead`](/dag_content/docs/4.x/api/composables/use-head) composable.
...
::tip
This [`useHead`](/dag_content/docs/4.x/api/composables/use-head) composable uses `@unhead/vue` under the hood (rather than `vue-meta`) to manipulate your `<head>`.
::

::warning
We recommend not using the native Nuxt 2 `head()` properties in addition to [`useHead`](/dag_content/docs/4.x/api/composables/use-head) , as they may conflict.
::

For more information on how to use this composable, see [the docs](/dag_content/docs/4.x/getting-started/seo-meta).
