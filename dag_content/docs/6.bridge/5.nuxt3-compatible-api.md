...
You can read more about how to use this composable in [the Nuxt 3 docs](/dag_content/docs/4.x/api/composables/use-state).
...
This function has been removed, but its use cases can be met by using [`useNuxtApp`](/dag_content/docs/4.x/api/composables/use-nuxt-app) or [`useState`](/dag_content/docs/4.x/api/composables/use-state) within `defineNuxtPlugin`. You can also run any custom code within the `setup()` function of a layout.
...
::warning
The `useLazyFetch` must have been configured for [Nitro](/dag_content/docs/4.x/bridge/nitro).
::
...
Nuxt Bridge also provides a Nuxt 3-compatible meta implementation that can be accessed with the [`useHead`](/dag_content/docs/4.x/api/composables/use-head) composable.
...
This will disable auto-imports completely but it's still possible to use [explicit imports](/dag_content/docs/4.x/bridge/nuxt3-compatible-api#explicit-imports) from `#imports`.
