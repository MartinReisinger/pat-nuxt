...
::note
To reduce your application's bundle size, this directory is **optional**, meaning that [`vue-router`](https://router.vuejs.org) won't be included if you only use [`app.vue`](/dag_content/docs/4.x/directory-structure/app/app). To force the pages system, set `pages: true` in `nuxt.config` or have a [`router.options.ts`](/dag_content/docs/4.x/guide/recipes/custom-routing#using-routeroptions).
::

## Usage

Pages are Vue components and can have any [valid extension](/dag_content/docs/4.x/api/nuxt-config#extensions) that Nuxt supports (by default `.vue`, `.js`, `.jsx`, `.mjs`, `.ts` or `.tsx`).
...
If you are using [`app.vue`](/dag_content/docs/4.x/directory-structure/app/app), make sure to use the [`<NuxtPage/>`](/dag_content/docs/4.x/api/components/nuxt-page) component to display the current page:
...
Pages **must have a single root element** to allow [route transitions](/dag_content/docs/4.x/getting-started/transitions) between pages. HTML comments are considered elements as well.
...
If you want to access the route using Composition API, there is a global [`useRoute`](/dag_content/docs/4.x/api/composables/use-route) function that will allow you to access the route just like `this.$route` in the Options API.
...
You can set a default value for this property [in your `nuxt.config`](/dag_content/docs/4.x/api/nuxt-config#keepalive).

#### `key`

[See above](/dag_content/docs/4.x/directory-structure/app/pages#child-route-keys).

#### `layout`

You can define the layout used to render the route. This can be either false (to disable any layout), a string or a ref/computed, if you want to make it reactive in some way. [More about layouts](/dag_content/docs/4.x/directory-structure/app/layouts).
...
You can set default values for these properties [in your `nuxt.config`](/dag_content/docs/4.x/api/nuxt-config#layouttransition).

#### `middleware`

You can define middleware to apply before loading this page. It will be merged with all the other middleware used in any matching parent/child routes. It can be a string, a function (an anonymous/inlined middleware function following [the global before guard pattern](https://router.vuejs.org/guide/advanced/navigation-guards#Global-Before-Guards)), or an array of strings/functions. [More about named middleware](/dag_content/docs/4.x/directory-structure/app/middleware).
...
To navigate between pages of your app, you should use the [`<NuxtLink>`](/dag_content/docs/4.x/api/components/nuxt-link) component.
...
You can define a page as [client only](/dag_content/docs/4.x/directory-structure/app/components#client-components) by giving it a `.client.vue` suffix. None of the content of this page will be rendered on the server.

## Server-Only Pages

You can define a page as [server only](/dag_content/docs/4.x/directory-structure/app/components#server-components) by giving it a `.server.vue` suffix. While you will be able to navigate to the page using client-side navigation, controlled by `vue-router`, it will be rendered with a server component automatically, meaning the code required to render the page will not be in your client-side bundle.
...
However, you can use [Nuxt Layers](/dag_content/docs/4.x/getting-started/layers) to create groupings of your app's pages:
