...
The first two kinds of route middleware can be defined in [`definePageMeta`](/dag_content/docs/4.x/api/utils/define-page-meta).
...
::note
Route middleware run within the Vue part of your Nuxt app. Despite the similar name, they are completely different from [server middleware](/dag_content/docs/4.x/directory-structure/server#server-middleware), which are run in the Nitro server part of your app.
::
...
1. [`navigateTo`](/dag_content/docs/4.x/api/utils/navigate-to) - Redirects to the given route
2. [`abortNavigation`](/dag_content/docs/4.x/api/utils/abort-navigation) - Aborts the navigation, with an optional error message.
...
Always use the `to` and `from` parameters in your middleware to access the next and previous routes. Avoid using the [`useRoute()`](/dag_content/docs/4.x/api/composables/use-route) composable in this context altogether.
...
It is possible to add global or named route middleware manually using the [`addRouteMiddleware()`](/dag_content/docs/4.x/api/utils/add-route-middleware) helper function, such as from within a plugin.
