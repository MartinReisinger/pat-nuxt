...
To add plugins in subdirectories, you can use the [`app/plugins`](/dag_content/docs/4.x/api/nuxt-config#plugins-1) option in `nuxt.config.ts`:
...
The only argument passed to a plugin is [`nuxtApp`](/dag_content/docs/4.x/api/composables/use-nuxt-app).
...
You can use [composables](/dag_content/docs/4.x/directory-structure/app/composables) as well as [utils](/dag_content/docs/4.x/directory-structure/app/utils) within Nuxt plugins:
...
Normally, Vue.js composables are bound to the current component instance while plugins are only bound to [`nuxtApp`](/dag_content/docs/4.x/api/composables/use-nuxt-app) instance.
::

## Providing Helpers

If you would like to provide a helper on the [`NuxtApp`](/dag_content/docs/4.x/api/composables/use-nuxt-app) instance, return it from the plugin under a `provide` key.
...
::important
Note that we highly recommend using [`composables`](/dag_content/docs/4.x/directory-structure/app/composables) instead of providing helpers to avoid polluting the global namespace and keep your main bundle entry small.
::
...
::note
If you need to use a provided helper _within_ another plugin, you can call [`useNuxtApp()`](/dag_content/docs/4.x/api/composables/use-nuxt-app) to get the typed version. But in general, this should be avoided unless you are certain of the plugins' order.
::
