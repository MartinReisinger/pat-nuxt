...
If you want to auto-import components from an npm package, you can use [`addComponent`](/dag_content/docs/4.x/api/kit/components#addcomponent) in a [local module](/dag_content/docs/4.x/directory-structure/modules) to register them.
...
By default, any file with an extension specified in the [extensions key of `nuxt.config.ts`](/dag_content/docs/4.x/api/nuxt-config#extensions) is treated as a component.
...
Server components can either be used on their own or paired with a [client component](/dag_content/docs/4.x/directory-structure/app/components#paired-with-a-client-component).
...
Server-only components use [`<NuxtIsland>`](/dag_content/docs/4.x/api/components/nuxt-island) under the hood, meaning that `lazy` prop and `#fallback` slot are both passed down to it.
...
You can use the [`addComponentsDir`](/dag_content/docs/4.x/api/kit/components#addcomponentsdir) method provided from the `@nuxt/kit` to register your components directory in your Nuxt module.
