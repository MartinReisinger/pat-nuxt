...
::note
When configuring a custom [`srcDir`](/dag_content/docs/4.x/api/nuxt-config#srcdir), make sure to place the `app.config` file at the root of the new `srcDir` path.
::
...
We can now universally access `theme` both when server-rendering the page and in the browser using [`useAppConfig`](/dag_content/docs/4.x/api/composables/use-app-config) composable.
...
The [`updateAppConfig`](/dag_content/docs/4.x/api/utils/update-app-config) utility can be used to update the `app.config` at runtime.
...
If you want to type the result of calling [`useAppConfig()`](/dag_content/docs/4.x/api/composables/use-app-config), then you will want to extend `AppConfig`.
...
Nuxt uses a custom merging strategy for the `AppConfig` within [the layers](/dag_content/docs/4.x/getting-started/layers) of your application.
