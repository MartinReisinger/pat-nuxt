...
::note
Note that currently server routes do not support the full functionality of dynamic routes as [pages](/dag_content/docs/4.x/directory-structure/app/pages#dynamic-routes) do.
::
...
By default, Nuxt 4 generates a [`tsconfig.json`](/dag_content/docs/4.x/directory-structure/tsconfig) which includes a project reference covering the `server/` folder which ensures accurate typings.
...
To return other error codes, throw an exception with [`createError`](/dag_content/docs/4.x/api/utils/create-error):
...
To return other status codes, use the [`setResponseStatus`](/dag_content/docs/4.x/api/utils/set-response-status) utility.
...
::note
Giving the `event` as argument to `useRuntimeConfig` is optional, but it is recommended to pass it to get the runtime config overwritten by [environment variables](/dag_content/docs/4.x/guide/going-further/runtime-config#environment-variables) at runtime for server routes.
::
...
Nitro provides a cross-platform [storage layer](https://nitro.build/guide/storage). In order to configure additional storage mount points, you can use `nitro.storage`, or [server plugins](/dag_content/docs/4.x/directory-structure/server#server-plugins).
