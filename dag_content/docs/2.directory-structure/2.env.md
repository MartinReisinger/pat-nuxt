...
::important
This file should be added to your [`.gitignore`](/dag_content/docs/4.x/directory-structure/gitignore) file to avoid pushing secrets to your repository.
::

## Dev, Build and Generate Time

Nuxt CLI has built-in [dotenv](https://github.com/motdotla/dotenv) support in development mode and when running [`nuxt build`](/dag_content/docs/4.x/api/commands/build) and [`nuxt generate`](/dag_content/docs/4.x/api/commands/generate).
...
::important
In your application code, you should use [Runtime Config](/dag_content/docs/4.x/guide/going-further/runtime-config) instead of plain env variables.
::
...
For local production preview purpose, we recommend using [`nuxt preview`](/dag_content/docs/4.x/api/commands/preview) since using this command, the `.env` file will be loaded into `process.env` for convenience. Note that this command requires dependencies to be installed in the package directory.
