...
You don't need to add those local modules to your [`nuxt.config.ts`](/dag_content/docs/4.x/directory-structure/nuxt-config) separately.
...
- First, the modules defined in [`nuxt.config.ts`](/dag_content/docs/4.x/api/nuxt-config#modules-1) are loaded.
