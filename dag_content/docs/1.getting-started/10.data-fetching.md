...
Nuxt comes with two composables and a built-in library to perform data-fetching in browser or server environments: `useFetch`, [`useAsyncData`](/dag_content/docs/4.x/api/composables/use-async-data) and `$fetch`.

In a nutshell:

- [`$fetch`](/dag_content/docs/4.x/api/utils/dollarfetch) is the simplest way to make a network request.
- [`useFetch`](/dag_content/docs/4.x/api/composables/use-fetch) is a wrapper around `$fetch` that fetches data only once in [universal rendering](/dag_content/docs/4.x/guide/concepts/rendering#universal-rendering).
- [`useAsyncData`](/dag_content/docs/4.x/api/composables/use-async-data) is similar to `useFetch` but offers more fine-grained control.
...
The [`useFetch`](/dag_content/docs/4.x/api/composables/use-fetch) and [`useAsyncData`](/dag_content/docs/4.x/api/composables/use-async-data) composables solve this problem by ensuring that if an API call is made on the server, the data is forwarded to the client in the payload.

The payload is a JavaScript object accessible through [`useNuxtApp().payload`](/dag_content/docs/4.x/api/composables/use-nuxt-app#payload). It is used on the client to avoid refetching the same data when the code is executed in the browser [during hydration](/dag_content/docs/4.x/guide/concepts/rendering#universal-rendering).
...
::note
You can add the [`<NuxtLoadingIndicator>`](/dag_content/docs/4.x/api/components/nuxt-loading-indicator) to add a progress bar between page navigations.
::
...
::warning
Beware that using only `$fetch` will not provide [network calls de-duplication and navigation prevention](/dag_content/docs/4.x/getting-started/data-fetching#the-need-for-usefetch-and-useasyncdata). :br
It is recommended to use `$fetch` for client-side interactions (event-based) or combined with [`useAsyncData`](/dag_content/docs/4.x/getting-started/data-fetching#useasyncdata) when fetching the initial component data.
::
...
When calling `useFetch` on the server, Nuxt will use [`useRequestFetch`](/dag_content/docs/4.x/api/composables/use-request-fetch) to proxy client headers and cookies (with the exception of headers not meant to be forwarded, like `host`).
...
::tip
You can also use [`useRequestFetch`](/dag_content/docs/4.x/api/composables/use-request-fetch) to proxy headers to the call automatically.
::
...
The [`useFetch`](/dag_content/docs/4.x/api/composables/use-fetch) composable uses `$fetch` under-the-hood to make SSR-safe network calls in the setup function.
...
This composable is a wrapper around the [`useAsyncData`](/dag_content/docs/4.x/api/composables/use-async-data) composable and `$fetch` utility.
...
It's developer experience sugar for the most common use case. (You can find out more about `event.fetch` at [`useRequestFetch`](/dag_content/docs/4.x/api/composables/use-request-fetch).)
::
...
There are some cases when using the [`useFetch`](/dag_content/docs/4.x/api/composables/use-fetch) composable is not appropriate, for example when a CMS or a third-party provide their own query layer. In this case, you can use [`useAsyncData`](/dag_content/docs/4.x/api/composables/use-async-data) to wrap your calls and still keep the benefits provided by the composable.
...
::note
The first argument of [`useAsyncData`](/dag_content/docs/4.x/api/composables/use-async-data) is a unique key used to cache the response of the second argument, the querying function. This key can be ignored by directly passing the querying function, the key will be auto-generated.
:br :br
...
:br :br
Setting a key can be useful to share the same data between components using [`useNuxtData`](/dag_content/docs/4.x/api/composables/use-nuxt-data) or to [refresh specific data](/dag_content/docs/4.x/api/utils/refresh-nuxt-data#refresh-specific-data).
::
...
::note
`useAsyncData` is for fetching and caching data, not triggering side effects like calling Pinia actions, as this can cause unintended behavior such as repeated executions with nullish values. If you need to trigger side effects, use the [`callOnce`](/dag_content/docs/4.x/api/utils/call-once) utility to do so.
...
You can alternatively use [`useLazyFetch`](/dag_content/docs/4.x/api/composables/use-lazy-fetch) and `useLazyAsyncData` as convenient methods to perform the same.
...
The `useFetch` composable is meant to be invoked in setup method or called directly at the top level of a function in lifecycle hooks, otherwise you should use [`$fetch` method](/dag_content/docs/4.x/getting-started/data-fetching#fetch).
...
[`useFetch`](/dag_content/docs/4.x/api/composables/use-fetch) and [`useAsyncData`](/dag_content/docs/4.x/api/composables/use-async-data) use keys to prevent refetching the same data.

- [`useFetch`](/dag_content/docs/4.x/api/composables/use-fetch) uses the provided URL as a key. Alternatively, a `key` value can be provided in the `options` object passed as a last argument.
...
::tip
To get the cached data by key, you can use [`useNuxtData`](/dag_content/docs/4.x/api/composables/use-nuxt-data)
::
...
The `execute` function is an alias for `refresh` that works in exactly the same way but is more semantic for cases when the fetch is [not immediate](/dag_content/docs/4.x/getting-started/data-fetching#not-immediate).

::tip
To globally refetch or invalidate cached data, see [`clearNuxtData`](/dag_content/docs/4.x/api/utils/clear-nuxt-data) and [`refreshNuxtData`](/dag_content/docs/4.x/api/utils/refresh-nuxt-data).
::
...
If you need to change the URL based on a reactive value, you may want to use a [computed URL](/dag_content/docs/4.x/getting-started/data-fetching#computed-url) instead.
...
Every time a dependency changes, the data will be fetched using the newly constructed URL. Combine this with [not-immediate](/dag_content/docs/4.x/getting-started/data-fetching#not-immediate), and you can wait until the reactive element changes before fetching.
...
If you need to force a refresh when other reactive values change, you can also [watch other values](/dag_content/docs/4.x/getting-started/data-fetching#watch).
