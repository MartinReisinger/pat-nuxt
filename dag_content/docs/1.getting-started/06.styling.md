...
If you're writing local stylesheets, the natural place to put them is the [`app/assets/` directory](/dag_content/docs/4.x/directory-structure/app/assets).
...
The natural place for your stylesheets is the [`app/assets/` directory](/dag_content/docs/4.x/directory-structure/app/assets). You can then reference its path and Nuxt will include it to all the pages of your application.
...
You can manipulate the head with the [`app.head`](/dag_content/docs/4.x/api/nuxt-config#head) property of your Nuxt configuration:
...
Nuxt modules provide you with a good developer experience out of the box, but remember that if your favorite tool doesn't have a module, it doesn't mean that you can't use it with Nuxt! You can configure it yourself for your own project. Depending on the tool, you might need to use a [Nuxt plugin](/dag_content/docs/4.x/directory-structure/app/plugins) and/or [make your own module](/dag_content/docs/4.x/guide/modules). Share them with the [community](/modules) if you do!
...
Nuxt comes with the same `<Transition>` element that Vue has, and also has support for the experimental [View Transitions API](/dag_content/docs/4.x/getting-started/transitions#view-transitions-api-experimental).
