...
- Errors during Nitro server lifecycle ([`server/`](/dag_content/docs/4.x/directory-structure/server) directory)
...
In addition, Nuxt provides a [`vue:error`](/dag_content/docs/4.x/api/advanced/hooks#app-hooks-runtime) hook that will be called if any errors propagate up to the top level.
...
This includes:
- running [Nuxt plugins](/dag_content/docs/4.x/directory-structure/app/plugins)
...
You cannot currently define a server-side handler for these errors, but can render an error page, see the [Render an Error Page](/dag_content/docs/4.x/getting-started/error-handling#error-page) section.
...
When you are ready to remove the error page, you can call the [`clearError`](/dag_content/docs/4.x/api/utils/clear-error) helper function, which takes an optional path to redirect to (for example, if you want to navigate to a 'safe' page).
...
::note
Rendering an error page is an entirely separate page load, meaning any registered middleware will run again. You can use [`useError`](/dag_content/docs/4.x/getting-started/error-handling#useerror) in middleware to check if an error is being handled.
::
...
- on server-side, it will trigger a full-screen error page which you can clear with [`clearError`](/dag_content/docs/4.x/getting-started/error-handling#clearerror).
...
You can call this function at any point on client-side, or (on server side) directly within middleware, plugins or `setup()` functions. It will trigger a full-screen error page which you can clear with [`clearError`](/dag_content/docs/4.x/getting-started/error-handling#clearerror).
...
Nuxt also provides a [`<NuxtErrorBoundary>`](/dag_content/docs/4.x/api/components/nuxt-error-boundary) component that allows you to handle client-side errors within your app, without replacing your entire site with an error page.
