...
Nuxt 3 provides new options for [fetching data from an API](/dag_content/docs/4.x/getting-started/data-fetching).
...
In Nuxt 3 you can use a globally available `fetch` method that has the same API as [the Fetch API](https://developer.mozilla.org/en-US/docs/Web/API/Fetch_API/Using_Fetch) or [`$fetch`](/dag_content/docs/4.x/api/utils/dollarfetch) method which is using [unjs/ofetch](https://github.com/unjs/ofetch). It has a number of benefits, including:

1. It will handle 'smartly' making [direct API calls](/dag_content/docs/4.x/guide/concepts/server-engine#direct-api-calls) if it's running on the server, or making a client-side call to your API if it's running on the client. (It can also handle calling third-party APIs.)

2. Plus, it comes with convenience features including automatically parsing responses and stringifying data.

You can read more [about direct API calls](/dag_content/docs/4.x/guide/concepts/server-engine#direct-api-calls) or [fetching data](/dag_content/docs/4.x/getting-started/data-fetching).

### Composables

Nuxt 3 provides new composables for fetching data: [`useAsyncData`](/dag_content/docs/4.x/api/composables/use-async-data) and `useFetch`. They each have 'lazy' variants (`useLazyAsyncData` and `useLazyFetch`), which do not block client-side navigation.
...
::note
Despite the names, [`useFetch`](/dag_content/docs/4.x/api/composables/use-fetch) is not a direct replacement of the `fetch()` hook. Rather, [`useAsyncData`](/dag_content/docs/4.x/api/composables/use-async-data) replaces both hooks and is more customizable; it can do more than simply fetching data from an endpoint. [`useFetch`](/dag_content/docs/4.x/api/composables/use-fetch) is a convenience wrapper around [`useAsyncData`](/dag_content/docs/4.x/api/composables/use-async-data) for simply fetching data from an endpoint.
::

### Migration

1. Replace the `asyncData` hook with [`useAsyncData`](/dag_content/docs/4.x/api/composables/use-async-data) or [`useFetch`](/dag_content/docs/4.x/api/composables/use-fetch) in your page/component.
2. Replace the `fetch` hook with [`useAsyncData`](/dag_content/docs/4.x/api/composables/use-async-data) or [`useFetch`](/dag_content/docs/4.x/api/composables/use-fetch) in your component.
...
You can now define a key within the [`definePageMeta`](/dag_content/docs/4.x/api/utils/define-page-meta) compiler macro.
...
This feature is not yet supported in Nuxt 3. If you want to overwrite the default scroll behavior of `vue-router`, you can do so in an `~/router.options.ts` (see [docs](/dag_content/docs/4.x/guide/recipes/custom-routing#router-options)) for more info.
Similar to `key`, specify it within the [`definePageMeta`](/dag_content/docs/4.x/api/utils/define-page-meta) compiler macro.
