...
If you have a fixture or example in your module, add `@nuxt/bridge` package to its config (see [example](/dag_content/docs/4.x/bridge/overview#update-nuxtconfig))
...
Your module should work even if it's only added to [`buildModules`](/dag_content/docs/4.x/api/nuxt-config#runtimeconfig) (instead of `modules`). For example:

- Avoid updating `process.env` within a Nuxt module and reading by a Nuxt plugin; use [`runtimeConfig`](/dag_content/docs/4.x/api/nuxt-config#runtimeconfig) instead.
...
::tip
Continue reading about Nuxt 3 modules in the [Modules Author Guide](/dag_content/docs/4.x/guide/modules).
::
