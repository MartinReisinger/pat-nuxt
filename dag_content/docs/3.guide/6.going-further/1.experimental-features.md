...
Internally, Nuxt uses `@nuxt/schema` to define these experimental features. You can refer to the [API documentation](/dag_content/docs/4.x/guide/going-further/experimental-features) or the [source code](https://github.com/nuxt/nuxt/blob/main/packages/schema/src/config/experimental.ts) for more information.
...
Allows Nuxt app state to be restored from `sessionStorage` when reloading the page after a chunk error or manual [`reloadNuxtApp()`](/dag_content/docs/4.x/api/utils/reload-nuxt-app) call.

To avoid hydration errors, it will be applied only after the Vue app has been mounted, meaning there may be a flicker on initial load.

::important
Consider carefully before enabling this as it can cause unexpected behavior,
and consider providing explicit keys to [`useState`](/dag_content/docs/4.x/api/composables/use-state) as auto-generated keys may not match across builds.
::
...
Define route rules at the page level using [`defineRouteRules`](/dag_content/docs/4.x/api/utils/define-route-rules).
...
Enables the experimental [`<NuxtClientFallback>`](/dag_content/docs/4.x/api/components/nuxt-client-fallback) component for rendering content on the client if there's an error in SSR.
...
You can also pass an object to configure [view transition types](/dag_content/docs/4.x/getting-started/transitions#view-transition-types), which allow different CSS animations based on the type of navigation:
...
Enables experimental component islands support with [`<NuxtIsland>`](/dag_content/docs/4.x/api/components/nuxt-island) and `.island.vue` files.
...
Out of the box, this will enable typed usage of [`navigateTo`](/dag_content/docs/4.x/api/utils/navigate-to), [`<NuxtLink>`](/dag_content/docs/4.x/api/components/nuxt-link), [`router.push()`](/dag_content/docs/4.x/api/composables/use-router) and more.
...
This allows modules to access additional metadata from the page metadata in the build context. If you are using this within a module, it's recommended also to [augment the `NuxtPage` types with your keys](/dag_content/docs/4.x/directory-structure/app/pages#typing-custom-metadata).
