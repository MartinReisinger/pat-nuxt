...
By default, Nuxt uses **universal rendering** to provide better user experience, performance and to optimize search engine indexing, but you can switch rendering modes in [one line of configuration](/dag_content/docs/4.x/api/nuxt-config#ssr).
...
[Middlewares](/dag_content/docs/4.x/directory-structure/app/middleware) and [pages](/dag_content/docs/4.x/directory-structure/app/pages) run in the server and on the client during hydration. [Plugins](/dag_content/docs/4.x/directory-structure/app/plugins) can be rendered on the server or client or both. [Components](/dag_content/docs/4.x/directory-structure/app/components) can be forced to run on the client only as well. [Composables](/dag_content/docs/4.x/directory-structure/app/composables) and [utilities](/dag_content/docs/4.x/directory-structure/app/utils) are rendered based on the context of their usage.
...
If you deploy your app to [static hosting](/dag_content/docs/4.x/getting-started/deployment#static-hosting) with the `nuxt generate` or `nuxt build --prerender` commands, then by default, Nuxt will render every page as a separate static HTML file.
...
In order to achieve this we can change how the routes are prerendered. Just add this to [your hooks](/dag_content/docs/4.x/api/advanced/hooks#nuxt-hooks-build-time) in your `nuxt.config.ts`:
...
::important
Note that Hybrid Rendering is not available when using [`nuxt generate`](/dag_content/docs/4.x/api/commands/generate).
::
...
Edge-side rendering is possible thanks to [Nitro](https://nitro.build/), the [server engine](/dag_content/docs/4.x/guide/concepts/server-engine) that powers Nuxt. It offers cross-platform support for Node.js, Deno, Cloudflare Workers, and more.
