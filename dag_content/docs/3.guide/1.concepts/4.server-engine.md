...
Server [API endpoints](/dag_content/docs/4.x/directory-structure/server#server-routes) and [Middleware](/dag_content/docs/4.x/directory-structure/server#server-middleware) are added by Nitro that internally uses [h3](https://github.com/h3js/h3).
...
Nitro allows 'direct' calling of routes via the globally-available [`$fetch`](/dag_content/docs/4.x/api/utils/dollarfetch) helper. This will make an API call to the server if run on the browser, but will directly call the relevant function if run on the server, **saving an additional API call**.

[`$fetch`](/dag_content/docs/4.x/api/utils/dollarfetch) API is using [ofetch](https://github.com/unjs/ofetch), with key features including:
...
You can access these types when using [`$fetch()`](/dag_content/docs/4.x/api/utils/dollarfetch) or [`useFetch()`](/dag_content/docs/4.x/api/composables/use-fetch).
...
Nuxt generates this dist when running `nuxt build` into a [`.output`](/dag_content/docs/4.x/directory-structure/output) directory.
