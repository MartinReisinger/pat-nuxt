...
By default, Nuxt doesn't check types when you run [`nuxt dev`](/dag_content/docs/4.x/api/commands/dev) or [`nuxt build`](/dag_content/docs/4.x/api/commands/build), for performance reasons.
...
Then, run [`nuxt typecheck`](/dag_content/docs/4.x/api/commands/typecheck) command to check your types:

```bash [Terminal]
npx nuxt typecheck
```

To enable type-checking at build or development time, you can also use the [`typescript.typeCheck`](/dag_content/docs/4.x/api/nuxt-config#typecheck) option in your `nuxt.config` file:
...
Nuxt projects rely on auto-generated types to work properly. These types are stored in the [`.nuxt`](/dag_content/docs/4.x/directory-structure/nuxt) directory and are generated when you run the dev server or build your application. You can also generate these files manually by running `nuxt prepare`.

The generated `tsconfig.json` files inside the [`.nuxt`](/dag_content/docs/4.x/directory-structure/nuxt) directory include **recommended basic TypeScript configuration** for your project, references to [auto-imports](/dag_content/docs/4.x/guide/concepts/auto-imports), [API route types](/dag_content/docs/4.x/guide/concepts/server-engine#typed-api-routes), path aliases like `#imports`, `~/file`, or `#build/file`, and more.

::warning
Nuxt relies on this configuration, and [Nuxt modules](/dag_content/docs/4.x/guide/modules) can extend it as well. For this reason, it is not recommended to modify your `tsconfig.json` file directly, as doing so could overwrite important settings. Instead, extend it via `nuxt.config.ts`. [Learn more about extending the configuration here](/dag_content/docs/4.x/directory-structure/tsconfig).
::
...
[Strict checks](https://www.typescriptlang.org/docs/handbook/migrating-from-javascript.html#getting-stricter-checks) are enabled by default in Nuxt when the [`typescript.typeCheck`](/dag_content/docs/4.x/guide/concepts/typescript#type-checking) option is enabled to give you greater type safety.
