...
- "local" modules exist within a Nuxt project, either [inlined in Nuxt config](/dag_content/docs/4.x/api/nuxt-config#modules) or within [the `modules` directory](/dag_content/docs/4.x/directory-structure/modules).
...
You can get type hinting for this function using the higher-level `defineNuxtModule` helper provided by [Nuxt Kit](/dag_content/docs/4.x/guide/going-further/kit).
...
- Vue components
- Composables
- [Nuxt plugins](/dag_content/docs/4.x/directory-structure/app/plugins)

To the [server engine](/dag_content/docs/4.x/guide/concepts/server-engine), Nitro:
- API routes
- Middlewares
- Nitro plugins
...
::tip
Learn more about asset injection in [the recipes section](/dag_content/docs/4.x/guide/modules/recipes-basics).
::
