...
If you need lower-level control, you can create a custom `$fetch` instance with a [Nuxt plugin](/dag_content/docs/4.x/directory-structure/app/plugins) and either use it with `useAsyncData` directly or pass it to `createUseFetch`.
...
::callout
Wrapping with [`useAsyncData`](/dag_content/docs/4.x/api/composables/use-async-data) **avoids double data fetching when doing server-side rendering** (server & client on hydration).
::
