...
::callout
You can hook into the underlying announcer instance using [the `useRouteAnnouncer` composable](/dag_content/docs/4.x/api/composables/use-route-announcer), which allows you to set a custom announcement message.
::

::callout
For announcing in-page content changes (form validation, toast notifications, loading states, etc.), use the [`<NuxtAnnouncer>`](/dag_content/docs/4.x/api/components/nuxt-announcer) component with the [`useAnnouncer`](/dag_content/docs/4.x/api/composables/use-announcer) composable instead.
::
