...
Usage of `<NuxtPicture>` is almost identical to [`<NuxtImg>`](/dag_content/docs/4.x/api/components/nuxt-img) but it also allows serving modern formats like `webp` when possible.
