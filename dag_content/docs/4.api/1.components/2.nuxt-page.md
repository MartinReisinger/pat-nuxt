...
`<NuxtPage>` is a built-in component that comes with Nuxt. It lets you display top-level or nested pages located in the [`app/pages/`](/dag_content/docs/4.x/directory-structure/app/pages) directory.
...
Alternatively, `pageKey` can be passed as a `key` value via [`definePageMeta`](/dag_content/docs/4.x/api/utils/define-page-meta) from the `<script>` section of your Vue component in the `/pages` directory.
