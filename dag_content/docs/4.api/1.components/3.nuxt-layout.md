...
- `name`: Specify a layout name to be rendered, can be a string, reactive reference or a computed property. It **must** match the name of the corresponding layout file in the [`app/layouts/`](/dag_content/docs/4.x/directory-structure/app/layouts) directory, or `false` to disable the layout.
  - **type**: `string | false`
  - **default**: `default`
...
- `fallback`: If an invalid layout is passed to the `name` prop, no layout will be rendered. Specify a `fallback` layout to be rendered in this scenario. It **must** match the name of the corresponding layout file in the [`app/layouts/`](/dag_content/docs/4.x/directory-structure/app/layouts) directory.
  - **type**: `string`
  - **default**: `null`
...
When using [`definePageMeta`](/dag_content/docs/4.x/api/utils/define-page-meta) with the object syntax for `layout`, props are automatically passed to the layout component. The layout can receive them with `defineProps`:
