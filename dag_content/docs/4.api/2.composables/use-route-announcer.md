...
A composable which observes the page title changes and updates the announcer message accordingly. Used by [`<NuxtRouteAnnouncer>`](/dag_content/docs/4.x/api/components/nuxt-route-announcer) and controllable.
...
::callout
For announcing dynamic in-page content changes (form validation, toasts, loading states), use [`useAnnouncer`](/dag_content/docs/4.x/api/composables/use-announcer) instead.
::
