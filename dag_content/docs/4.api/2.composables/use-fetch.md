...
This composable provides a convenient wrapper around [`useAsyncData`](/dag_content/docs/4.x/api/composables/use-async-data) and [`$fetch`](/dag_content/docs/4.x/api/utils/dollarfetch).
...
::tip
Keyed state created using `useFetch` can be retrieved across your Nuxt application using [`useNuxtData`](/dag_content/docs/4.x/api/composables/use-nuxt-data).
::

::warning
`useFetch` is a reserved function name transformed by the compiler, so you should not name your own function `useFetch`. To create a custom variant with pre-defined options, use [`createUseFetch`](/dag_content/docs/4.x/guide/recipes/custom-usefetch#custom-usefetch-with-createusefetch) instead.
::
...
- `options` (object): Configuration for the fetch request. Extends [unjs/ofetch](https://github.com/unjs/ofetch) options and [`AsyncDataOptions`](/dag_content/docs/4.x/api/composables/use-async-data#params). All options can be a static value, a `ref`, or a computed value.
...
| `$fetch`        | `typeof globalThis.$fetch`                                              | -          | Custom $fetch implementation. See [Custom useFetch in Nuxt](/dag_content/docs/4.x/guide/recipes/custom-usefetch)             |
