...
::tip
The [`useFetch`](/dag_content/docs/4.x/api/composables/use-fetch) composable uses `useRequestFetch` under the hood to automatically forward the request context and headers.
::
...
::tip
In the browser during client-side navigation, `useRequestFetch` will behave just like regular [`$fetch`](/dag_content/docs/4.x/api/utils/dollarfetch).
::
