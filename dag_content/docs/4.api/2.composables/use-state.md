...
- `key`: A unique key ensuring that data fetching is properly de-duplicated across requests. If you do not provide a key, then a key that is unique to the file and line number of the instance of [`useState`](/dag_content/docs/4.x/api/composables/use-state) will be generated for you.
...
If you want to store class instances with `useState` that are not supported by Nuxt, you can use [`definePayloadPlugin`](/dag_content/docs/4.x/api/composables/use-nuxt-app#custom-reducerreviver) to add a custom serializer and deserializer for your classes.
