...
`usePreviewMode` will try to store the value of a `token` param from url in state. You can modify this state and it will be available for all [`usePreviewMode`](/dag_content/docs/4.x/api/composables/use-preview-mode) calls.
...
::note
`usePreviewMode` should be tested locally with `nuxt generate` and then `nuxt preview` rather than `nuxt dev`. (The [preview command](/dag_content/docs/4.x/api/commands/preview) is not related to preview mode.)
::
