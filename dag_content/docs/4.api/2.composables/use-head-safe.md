...
`input`: A `MaybeComputedRef<HeadSafe>` object containing head data. You can pass all the same values as [`useHead`](/dag_content/docs/4.x/api/composables/use-head), but only safe attributes will be rendered.
