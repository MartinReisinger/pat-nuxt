...
::tip
This is particularly useful when you want to disable modules inherited from [Nuxt layers](/dag_content/docs/4.x/guide/going-further/layers#disabling-modules-from-layers).
::
...
**Deprecated:** Use the [`moduleDependencies`](/dag_content/docs/4.x/api/kit/modules#specifying-module-dependencies) option in `defineNuxtModule` instead. The `installModule` function will be removed (or may become non-blocking) in a future version.
::
