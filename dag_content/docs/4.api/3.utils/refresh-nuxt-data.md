...
`refreshNuxtData` is used to refetch all or specific `asyncData` instances, including those from [`useAsyncData`](/dag_content/docs/4.x/api/composables/use-async-data), [`useLazyAsyncData`](/dag_content/docs/4.x/api/composables/use-lazy-async-data), [`useFetch`](/dag_content/docs/4.x/api/composables/use-fetch), and [`useLazyFetch`](/dag_content/docs/4.x/api/composables/use-lazy-fetch).  
...
* `keys`: A single string or an array of strings as `keys` that are used to fetch the data. This parameter is **optional**. All [`useAsyncData`](/dag_content/docs/4.x/api/composables/use-async-data) and [`useFetch`](/dag_content/docs/4.x/api/composables/use-fetch) keys are re-fetched when no `keys` are explicitly specified.
