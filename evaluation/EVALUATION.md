...
Nuxt 4-specific API information, since Nuxt v4 came out on Jul 16, 2025, and the model's knowledge-cutoff date is Jan.
2025.
...
1. *Wrong section retrieved* When searching for generic words like `hooks, layout, page` matched multiple irrelevant
   sections (like Vite plugin hook documentation), pushing the relevant ones (`pages:resolved` in this case) outside the
   set retrieval window. This could probably be improved by:
    1. A bigger retrieval-window and a re-ranking algorithm, so relevant parts do not get pushed out of the retrieval.
    2. Using semantic-vector-embedding instead of s a simple grep search, better encoding the relevant concept.
2. *Conceptual shortfalls* Tasks where one would have to know the name of the API before you can search it. Like task
   3 (to use normalized component names for `KeepAlive`), task 5 (`noUncheckedIndexedAccess`) or task 19 (
   `spaLoaderAttrs`). But these all involve configuration keys or behavioral changes, so even a developer would not know
   about those, unless they already knew about the change.

### Perfect Info (92%)

Here the exact documentation about the relevant change from the official Nuxt v4 Upgrade Guide was injected in every
...
1. *Task 24* — The correct solution calls `addTemplate()` from `@nuxt/kit` with a `getContents` callback to
   ...
   `getContents` but reached for `nuxt.options.build.templates.push` instead — indicating this API requires broader
   module-kit context that no single documentation section fully captures. Notably, all three conditions failed on this
   task. The correct output:
   ```js
   addTemplate({
...
2. *Task 25* — The correct output is the complete `app/` directory tree with middleware grouped into named subfolders,
   ...
   root or the `index.ts` files inside each subfolder. This may reflect ambiguity in the task specification rather than
   a retrieval failure, since the Simple DAG condition passed. The correct output:
   ```
   .
   ├── app/
   │   ├── middleware/
...
- *Non-determinism.* Because LLMs are stochastic by nature, re-running the exact same experiment would probably produce
  a
...
- *Small sample size.* Our experiment has only 25 tasks, meaning each percentage point amounts to 0.25 tasks. Or, to put
  it differently: a single answer being different could swing the pass rate by 4%.
- *Single framework.* We consciously scoped our experiment to Nuxt (a Vue-based full-stack framework), specifically the
